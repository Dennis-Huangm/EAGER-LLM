...
def extract_numbers(text):
    ...
    for i in range(len(text_list)):
        text_num[i] = int(re.search(r'\d+', text_list[i]).group())
    return text_num

def match_codes(prediction,target_item):
    if (prediction == target_item):
        return True
    if(prediction.find("unk")!=-1):
        pred_num = extract_numbers(prediction)
        target_num = extract_numbers(target_item)
        cnt=0
        for i in range(len(pred_num)):
            if pred_num[i] != target_num[i]:
                cnt+=1
        if cnt<=1:
            print("prediction:{} target_item:{}".format(prediction,target_item))
            return True
    return False
...
def get_topk_results(predictions, scores, targets, k, all_items=None):
    ...
    for b in range(B):
        ...
        for sorted_pred in sorted_pairs:
            if match_codes(sorted_pred[0],target_item) and (sorted_pred[1] > -1000):
