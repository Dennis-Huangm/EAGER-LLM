...
def load_reviews(file, rating_score=0):
    '''
    ...
    for inter in tqdm(parse(file), desc='Load reviews'):
        if float(inter['overall']) <= rating_score: # 小于一定分数去掉
            continue
        users.add(inter['reviewerID'])
        items.add(inter['asin'])
        inters.add((inter['reviewerID'], inter['asin'], float(inter['overall']), int(inter['unixReviewTime'])))
    return users, items, inters

def load_meta_items(file):
    items = {}
    for line in tqdm(parse(file), desc="Load metas"):
        data = line
        item = data["asin"]
...
def load_review_data(args, user2id, item2id):
    ...
    review_file_path = os.path.join(args.input_path, 'Review', 'reviews_' + dataset_full_name + '_5.json.gz')
...
def preprocess_review(args):
    ...
    # load ratings
    review_file_path = os.path.join(args.input_path, 'Review', 'reviews_' + dataset_full_name + '_5.json.gz')
    rating_users, rating_items, rating_inters = load_reviews(review_file_path)
...
if __name__ == '__main__':
    ...
    rating_inters, meta_items = preprocess_review(args) 
