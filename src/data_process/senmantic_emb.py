...
def preprocess_text(args):
    ...
    item_text_list = generate_text(item2feature, ['title', 'description','categories'])
...
def generate_item_embedding(args, item_text_list, tokenizer, model, word_drop_ratio=-1):
    ...
    with torch.no_grad():
        with tqdm(total=len(order_texts)) as pbar:
            while start < len(order_texts):
                ...
                for sentences in field_texts:
                    ...
                    masked_output = outputs.last_hidden_state * \
                        encoded_sentences['attention_mask'].unsqueeze(-1)
                    mean_output = masked_output.sum(
                        dim=1) / encoded_sentences['attention_mask'].sum(dim=-1, keepdim=True)
                    mean_output = mean_output.detach().cpu()
                    field_embeddings.append(mean_output)
                field_mean_embedding = torch.stack(
                    field_embeddings, dim=0).mean(dim=0)
    ...
    file = os.path.join(args.root, args.dataset + '.emb-' +
                        args.plm_name + "-td" + ".npy")
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='Beauty',
                        help='Instruments / Beauty / Sports')
    parser.add_argument('--root', type=str, default="")
    parser.add_argument('--gpu_id', type=int, default=2,
                        help='ID of running GPU')
