...
def cut(_input, _train, _test, _validation, _item_vec, _number):
    ...
    datas = []
    file = open(_input, 'r')
    for line in tqdm(file, desc="Load metas"):
        dic = json.loads(line)
        datas.append(dic)
...
def _read(raw_file, test_record_num):
    ...
    # record the behavior type, 5 types, key is the type, value is the id of the type
    behavior_dict = dict()
    # record user id, item id, cate_id, behavior_id, timestampe. key the liks 'USERID', value is a array.
    train_sample = dict()
    test_sample = dict()
    ...
    with open(filename, 'w') as f:
        for user in users:
            ...
            for i in range(len(arr) - seq_len+1):
                ...
                f.write("{}".format(
                    ",".join([str(v) for v in arr[i+seq_len-1:i+seq_len-1+int(seq_len/5)]])))
                f.write('\n')
                count += 1
    # pipe.send((count))
