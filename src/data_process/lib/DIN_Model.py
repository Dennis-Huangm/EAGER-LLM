...
class DeepInterestNetwork(nn.Module):
    def __init__(self, item_num=100,embedding_dim=128,
        ...
        self.item_embedding=\
            EmbeddingLayer(item_num,embedding_dim)
            # nn.Linear(768, 256),
            # nn.ReLU(inplace=True),
            # nn.Linear(256, embedding_dim))
        self.magnification=self.embed_dim//128
        self.attention_unit=LocalActivationUnit(hidden_size=[64*self.magnification, 16*self.magnification], bias=[True, True], \
                                            embedding_dim=embedding_dim, batch_norm=False)
        if sum_pooling:    
            self.fc_layer = FullyConnectedLayer(input_size=2*embedding_dim,
                                    hidden_size=[200*self.magnification, 80*self.magnification, 1],
                                    bias=[True, True, True],
                                    activation='dice',
                                    sigmoid=False)    
        else:                               
