...
class BaseDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        self.data_path = os.path.join(args.data_path, self.dataset, args.encoder_type, args.date)
    ...
    def get_all_items_without_dup(self):
        ...
        for index in self.indices.values():
            self.all_items.add("".join(index))
            if "".join(index) in exist_items:
                self.all_items.remove("".join(index))
                print("duplicate item: ", "".join(index))
                cnt+=1
            exist_items.add("".join(index))
        print("duplicate item num : ",cnt)
        print("all item num : ",len(self.all_items))
        return self.all_items
        ...
        if self.allowed_tokens is None:
            ...
            for index in self.indices.values():
                for i, token in enumerate(index):
                    token_id = tokenizer(token)["input_ids"][1] # llama
                    # token_id = tokenizer(token)["input_ids"][2] # llama2
...
class SeqRecDataset(BaseDataset):
    ...
    def _load_data(self):
        ...
        with open(os.path.join(self.data_path, self.dataset + self.index_file), 'r') as f:
            self.indices = json.load(f)
            self.token_2_idx = { "".join(indices): idx for idx, indices in self.indices.items() }
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            emb_id = self.token_2_idx[d['item']]
        elif self.mode == 'test':
        ...
        if self.mode == 'train':
            return dict(input_ids=input, labels=output, emb_id=emb_id)    
